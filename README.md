...
集成 TUIOfflinePush 组件之前，需要先向 Apple [申请 APNs 推送证书](https://cloud.tencent.com/document/product/269/44517#.E6.AD.A5.E9.AA.A41.EF.BC.9A.E7.94.B3.E8.AF.B7-apns-.E8.AF.81.E4.B9.A6)，然后 [上传推送证书到 IM 控制台](https://cloud.tencent.com/document/product/269/44517#.E6.AD.A5.E9.AA.A42.EF.BC.9A.E4.B8.8A.E4.BC.A0.E8.AF.81.E4.B9.A6.E5.88.B0.E6.8E.A7.E5.88.B6.E5.8F.B0) 。之后按照如下步骤操作即可快速接入 IM 离线推送。

2. [集成 TUIOfflinePush 组件](#pod_tuiofflinepush)
3. [配置推送参数](#push_para)
...
> ?  如果您想尽可能简单地接入 TUIOfflinePush 组件，您需要使用 TUICore 组件中的 TUILogin 提供的 login/logout 接口登录/登出，此时 TUIOfflinePush 组件会自动感知登录/登出事件。如果您不想使用 TUILogin 提供的接口，可参考 TUIOfflinePush 的 [高级用法-自定义登录/登出](#advance_login_logout)。
...
当您 [上传证书到 IM 控制台](#upload_certification) 后，IM 控制台会为您分配一个 [证书ID](#businessid) ，见下图。

<img src="https://im.sdk.cloud.tencent.cn/tools/resource/offlinepush/ios/businessID.png" style="zoom:40%;" />



您需要在 AppDelegate 中，调用 `TUIOfflinePushCertificateIDForAPNS` 设置下证书 ID 即可。
...
> ? `TUIOfflinePushCertificateIDForAPNS` 需要在 `AppDelegate` 的 `@implementation` 中调用。
...
点击通知栏的离线推送后，TUIOfflinePush 组件已支持推送内容的解析。

如果要实现跳转到聊天列表，您只需要在 AppDelegate 中实现 `-navigateToTUIChatViewController:groupID:` 跳转方法即可。
